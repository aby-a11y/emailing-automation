...
def get_db_path():
    """Har baar fresh DB path lo - .exe aur Python dono ke liye"""
    if os.environ.get("OUTREACHOS_DB"):
        return os.environ.get("OUTREACHOS_DB")
    elif getattr(sys, 'frozen', False):
        return os.path.join(os.path.dirname(sys.executable), "outreachos.db")
    else:
        _script_dir = os.path.dirname(os.path.abspath(__file__))
        _dist_db = os.path.join(_script_dir, "dist", "outreachos.db")
        _local_db = os.path.join(_script_dir, "outreachos.db")
        if os.path.exists(_dist_db) and (not os.path.exists(_local_db) or os.path.getsize(_dist_db) > os.path.getsize(_local_db)):
            return _dist_db
        return _local_db
...
def load_status():
    ...
    cur.close()
    conn.close()
    print(f"[INFO] DB se {len(rows)} leads load hue")
    return rows
...
def connect_server():
    try:
        # Port 465 SSL - local machine ke liye best
        server = smtplib.SMTP_SSL("smtp.gmail.com", 465)
        server.login(EMAIL, PASSWORD)
        print("[OK] Gmail SMTP connected (port 465)")
        return server
    except Exception as e:
        print(f"[ERR] SMTP 465 failed: {e}, trying 587...")
        try:
            ...
            server.ehlo()
            server.login(EMAIL, PASSWORD)
            print("[OK] Gmail SMTP connected (port 587)")
            return server
        except Exception as e2:
            print(f"[ERR] SMTP failed completely: {e2}")
            raise

def send_email(server, to_email, name, subject, body):
    # UTF-8 encoding explicitly set karo Windows crash se bachne ke liye
...
def run(stop_flag=None):
    ...
    if not leads:
        print("[WARN]  DB mein koi leads nahi hain. Pehle dashboard se CSV upload karo.")
        return
    ...
    for row in leads:
        ...
        if stop_flag and stop_flag.is_set():
            print("[STOP] Stop flag - automation band")
            break
        ...
        if str(row["replied"]).strip().upper() == "TRUE":
            print(f"  [SKIP]  Skipped (replied): {email}")
            continue
        ...
        if sent_count > 0 and sent_count % 15 == 0:
            ...
            server = connect_server()
            print("[RECONNECT] Server reconnected")
